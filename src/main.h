...
static const int LAST_POW_BLOCK = 554; // There will only be 554 Blocks to be mined
...
static const int64_t MAX_MONEY = 357 * COIN; // There will only be 357 Proof of Work coins
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.05 * COIN; // 5% interest per annum
static const int64_t COIN_YEAR_REWARD = 20 * CENT; // 5% interest per annum.
...
static const uint256 hashGenesisBlock("0x0000007e85f13de21720bd0f653285e8a5a420ea2435eda530705a2b013ea636");
...
public:
    ...
    {
        // learncoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // learncoin: get transaction coin age
...
public:
    ...
    // learncoin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // learncoin: two types of block: proof-of-work or proof-of-stake
    ...
    // learncoin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // learncoin: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // learncoin: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // learncoin: block index flags
    enum  
    {
