...
unsigned int nTargetSpacing = 1 * 60; // A block will be found every 60 seconds
unsigned int nRetarget = 20;
unsigned int nStakeMinAge = 24 * 60 * 60; // Coins need to be in the wallet for at least 24 hours before they start staking.
unsigned int nStakeMaxAge = -1; // It will stake forever
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // learncoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // learncoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    int64_t nSubsidy = 1 * COIN;
        ...
        if(pindexBest->nHeight < 10) // This method will apply to all blocks lower than 10 (Blocks 1-9)
        {
            nSubsidy = 10 * COIN; // They will receive 10 coins
        }
        else if(pindexBest->nHeight < 555) // This method will apply to all blocks lower than 555 and not referred to in any other code (Blocks 10-554)
        {
            nSubsidy = 0.5 * COIN; // They will receive half a coin
        }
...
// learncoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes learncoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // learncoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // learncoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // learncoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // learncoin: track money supply and mint amount info
...
// learncoin: total coin age spent in transaction, in the unit of coin-days.
...
// learncoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // learncoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // learncoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // learncoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // learncoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // learncoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // learncoin: check proof-of-stake
    ...
    // learncoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // learncoin: getblocks may not obtain the ancestor block rejected
    ...
    // learncoin: if responsible for sync-checkpoint send it
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Learns Learns Learns"; // Small phrase or title
        CTransaction txNew;
        txNew.nTime = 1405840372; // It's currently Sun, 20 Jul 2014 07:12:52 GMT here in Sydney
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1405840372; // It's currently Sun, 20 Jul 2014 07:12:52 GMT here in Sydney
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x37764c6a21d7bebc1239fa900cc6b4dba2583f4eda959fc1d4321889df6a9127"));
        ...
        // learncoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0xf9, 0xbe, 0xb4, 0xd };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // learncoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // learncoin: tell downloading node about the latest block if it's
